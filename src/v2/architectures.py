...
class StackedMapFeaturesExtractor(BaseFeaturesExtractor):
    ...
    across scales do not correspond. Fuses map features with sensor (lidar)
    features.
    """
    ...
    def __init__(
        ...
        # Conv2d(2x2, stride=2) -> 3x3 -> 3x3 -> 3x3 -> flatten -> Linear -> ReLU
        # Output spatial size: (map_size//2 - 2 - 2 - 2)^2 * out_channels
    ...
    def forward(self, observations: dict) -> th.Tensor:
        ...
        # Concatenate map types along channel dim: batch x (3*num_maps) x W x H
        maps = th.cat([coverage, obstacles, frontier], dim=1)

        # Group by scale: permute so scales become the group dimension
        # from: batch x (3*num_maps) x W x H
        # to:   batch x num_maps x 3 x W x H
        b, _, w, h = maps.shape
        num_maps = maps.shape[1] // 3
        maps = maps.reshape(b, num_maps, 3, w, h)
        # to:   batch x (num_maps*3) x W x H (groups=num_maps processes each scale's 3 channels independently)
        maps = maps.reshape(b, num_maps * 3, w, h)
