...
def draw_env_view(surface, env, x0, y0):
    ...
    # Flip vertically (y-axis) and convert BGR->RGB
...
def main():
    ...
    pygame.display.set_caption("Robot Teleop & Debug Viewer v2")
    ...
    while running:
        ...
        for i, name in enumerate(SENSOR_NAMES[:6]):
            ...
            draw_bar(screen, panel_x + 110, y + 2, 100, 10, val, 3.5, color)
            y += 16

        draw_text(
            screen,
            f" min_frt: {sensors[6]:.3f}",
            panel_x + 10,
            y,
            font_sm,
            (200, 200, 200),
