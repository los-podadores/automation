#!/usr/bin/env python3
"""
Run the C* controller on RobotCoverageEnvNoLimit for testing and visualization.

Usage:
    uv run python src/algo/run_cstar.py                  # phase 1
    uv run python src/algo/run_cstar.py --phase 3        # phase 3
    uv run python src/algo/run_cstar.py --render human   # with pygame display
    uv run python src/algo/run_cstar.py --episodes 5     # run 5 episodes
"""
...
from cstar_controller import CStarController
...
def run_episode(env, controller, render=False, max_steps=50000):
    ...
    for step in range(max_steps):
        ...
        if step % 500 == 0 and step > 0:
            print(
                ...
                f"coll={info['num_collisions']}  nodes={len(controller.rcg.nodes)}",
                flush=True,
            )
    ...
    return {
        ...
        "truncated": truncated,
        "rcg_nodes": len(controller.rcg.nodes),
        "rcg_edges": sum(len(v) for v in controller.rcg.edges.values()) // 2,
    }


def main():
    parser = argparse.ArgumentParser(description="Run C* controller")
    ...
    parser.add_argument("--w", type=float, default=0.9, help="Sampling resolution (lap spacing)")
    ...
    controller = CStarController(
        env, w=args.w,
        kp_heading=2.0, kp_throttle=1.0,
    )
    ...
    for ep in range(args.episodes):
        ...
        print(
            ...
            f"rcg_nodes={r['rcg_nodes']}, rcg_edges={r['rcg_edges']}, "
    ...
    if len(results) > 1:
        ...
        avg_nodes = np.mean([r["rcg_nodes"] for r in results])
        ...
        print(f"  RCG Nodes:  {avg_nodes:.0f}")
