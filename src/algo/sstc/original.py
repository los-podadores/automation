...
class SpiralSpanningTreeCoveragePlanner:
    def __init__(self, occ_map):
        ...
        if self.origin_map_height % 2 == 1 or self.origin_map_width % 2 == 1:
            sys.exit(
                "original map width/height must be even \
    ...
    def plan(self, start):
        ...
        for idx in range(len(route) - 1):
            dp = abs(route[idx][0] - route[idx + 1][0]) + abs(
    ...
    def get_intermediate_node(self, p, q):
        ...
        if len(itsc) == 0:
            sys.exit(
                "get_intermediate_node: \
                 no intermediate node between",
                p,
                q,
            )
        elif len(itsc) == 1:
            ...
            sys.exit(
                "get_intermediate_node: \
                more than 1 intermediate node between",
                p,
                q,
            )
